...
namespace ExpandedAiFramework
{
    public class WeightedTypePicker<T>
    {
        ...
        public Type PickType(T t)
        {
            ...
            {
                LogVerbose($"Checking {entry.Type} in WeightedTypePicker. Condition is {entry.Condition(t)}, weight is {entry.WeightProvider()}");
                if (entry.Condition(t))
                {
                    ...
                    if (weight > 0)
                    {
                        ...
                        LogVerbose($"Add {entry.Type} to WeightedTypePicker valid pool. Compiled pool weight is now {totalValidWeight} with {validEntries.Count} entries.");
                    }
                }
            }
            ...
            LogVerbose($"Rolled {roll}, checking against pool.");
            foreach (var (type, weight) in validEntries)
            {
                cumulative += weight;
                LogVerbose($"Added {type}, cumulative weight is {cumulative}");
                if (roll <= cumulative)
                {

                    LogVerbose($"Cumulative weight {cumulative} >=roll {roll}, picking type {type}!");
                    return type;
                }
            }
